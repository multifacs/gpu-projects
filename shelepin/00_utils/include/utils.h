...
void generateVector(T* data, const size_t size) {
  ...
  for (int i = 0; i < size; ++i) data[i] = dist(rd);
}
...
void generateSymmetricPositiveMatrix(T* matrix, int size) {
  ...
  static std::default_random_engine re(rd());
  static std::uniform_real_distribution<T> dist{2.f, 3.f};
  static std::uniform_real_distribution<T> dist_diag{size * 3.f,
                                                     size * 3.f + 1.f};

  for (size_t i = 0; i < size; ++i) {
    for (size_t j = 0; j < size; ++j) {
      float tmp = dist(rd);
      if (i != j) matrix[i * size + j] = tmp;
    }
...
template <typename T>
void checkSolutionOfSOLE(const size_t size, const T* A, const T* b, const T* x,
                         const float eps) {
  std::cout << std::fixed;
  std::cout.precision(6);
  ...
  std::cout << "[ CHECK ]  EPS - Achieved: " << accuracy << "\t"
            << "EPS - Expected: " << eps << std::endl;
