...
void matmul(const size_t m, const size_t n, const size_t k, const float* a,
...
void matmul_omp(const size_t m, const size_t n, const size_t k, const float* a,
                const float* b, float* c) {
  std::memset(c, 0, n * k * sizeof(float));
...
void matmul_cl(const size_t m, const size_t n, const size_t k, const float* a,
               const float* b, float* c,
  ...
  cl_program program =
      createProgramFromSource(context, "kernels/matmul_kernel.cl");
  CONTROL("clBuildProgram", clBuildProgram(program, 1, &dev_pair.second,
  ...
  CONTROL("clEnqueueWriteBuffer A",
          clEnqueueWriteBuffer(queue, a_buffer, CL_TRUE, 0,
                               sizeof(float) * m * n, a, 0, nullptr, nullptr));
  CONTROL("clEnqueueWriteBuffer B",
          clEnqueueWriteBuffer(queue, b_buffer, CL_TRUE, 0,
                               sizeof(float) * n * k, b, 0, nullptr, nullptr));

  CONTROL("clSetKernelArg M",
          clSetKernelArg(kernel, 0, sizeof(unsigned int), &m));
  CONTROL("clSetKernelArg N",
          clSetKernelArg(kernel, 1, sizeof(unsigned int), &n));
  CONTROL("clSetKernelArg K",
          clSetKernelArg(kernel, 2, sizeof(unsigned int), &k));
  CONTROL("clSetKernelArg A",
          clSetKernelArg(kernel, 3, sizeof(cl_mem), &a_buffer));
  CONTROL("clSetKernelArg B",
          clSetKernelArg(kernel, 4, sizeof(cl_mem), &b_buffer));
  CONTROL("clSetKernelArg C",
          clSetKernelArg(kernel, 5, sizeof(cl_mem), &c_buffer));

  size_t group = 16;
  CONTROL("clGetKernelWorkGroupInf",
          clGetKernelWorkGroupInfo(kernel, dev_pair.second,
  ...
  CONTROL("clEnqueueNDRangeKernel",
          clEnqueueNDRangeKernel(queue, kernel, ndims, nullptr, global, local,
                                 0, nullptr, nullptr));
...
void gemm_cl(const size_t m, const size_t n, const size_t k, const float* a,
             const float* b, float* c,
             std::pair<cl_platform_id, cl_device_id>& dev_pair, timer& time) {
  cl_int error = CL_SUCCESS;
  cl_context_properties properties[3] = {
  ...
  cl_program program =
      createProgramFromSource(context, "kernels/gemm_kernel.cl");
  std::string build_options = "-DBLOCK=" + std::to_string(BLOCK);
  CONTROL("clBuildProgram",
          clBuildProgram(program, 1, &dev_pair.second, build_options.c_str(),
                         nullptr, nullptr));
